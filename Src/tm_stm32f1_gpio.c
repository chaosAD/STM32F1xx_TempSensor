...
void TM_GPIO_SetPullResistor(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, TM_GPIO_PuPd_t GPIO_PuPd) {
    ...
    TARUC_GPIO_SetPullType(GPIOx, GPIO_PuPd, pinpos);
	}
}
...
void TM_GPIO_INT_Init(GPIO_TypeDef* GPIOx,                                  \
    ...
    TARUC_GPIO_SetPullType(GPIOx, GPIO_PuPd, pinpos);
		...
		TARUC_GPIO_SetMode(GPIOx, GPIO_Mode, pinpos);

		/* Set only if output or alternate functions */
		if (GPIO_Mode == TM_GPIO_Mode_OUT || GPIO_Mode == TM_GPIO_Mode_AF) {
			/* Set GPIO OTYPE register */
			TARUC_GPIO_SetOutputType(GPIOx, GPIO_OType, pinpos);

			/* Set GPIO OSPEED register */
			TARUC_GPIO_SetSpeed(GPIOx, GPIO_Speed, pinpos);
		}
	}
}
